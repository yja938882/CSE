...
struct node{
    ...
    int cost; // 현재 노드 까지의 코스트
    node* prev; // 부모 노드
};

struct CompareNode{
    bool operator()(node* a, node* b){
        return a->heuristic + a->cost > b->heuristic + b->cost; // g(n) + h(n)
    }
};
...
int ASS( int **, int, int, vector< pair<int, int> > );
...
int main(int argc, char* argv[]){
    ...
    int length = ASS( arr, N, M, dest); // A* Search
...
*/
int ASS( int **arr ,int N, int M, vector< pair<int, int> > dest){
